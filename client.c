    ...
    while (1) {
        ...
        if (n > 0) {
            ...
            for (int i = s; i != e; i = (i + 1) % WND_SIZE) {
                if ((pkts[i].seqnum + pkts[i].length) % MAX_SEQN == ackpkt.acknum) {
                    temp = i;
                    break;
                }
            }
            if (full && (pkts[e].seqnum + pkts[e].length) % MAX_SEQN == ackpkt.acknum) {
                temp = e;
            }
            ...
            for (int i = s; i != e; i = (i + 1) % WND_SIZE) {
                if (!pkts[i].dupack && isTimeout(timers[i])) {
                    printTimeout(&pkts[i]);
                    printSend(&pkts[i], 1);
            ...
            if (full && !pkts[e].dupack && isTimeout(timers[e])) {
                printTimeout(&pkts[e]);
                printSend(&pkts[e], 1);
                sendto(sockfd, &pkts[e], PKT_SIZE, 0, (struct sockaddr*) &servaddr, servaddrlen);
                timers[e] = setTimer();
