    ...
    for (int i = 1; ; i++) {
        ...
        while(1) {
            ...
            if (n > 0) {
                ...
                if (!recvpkt.fin) {
                    // save packet
                    int diff = recvpkt.seqnum - cliSeqNum;
                    if (diff < 0) {
                        diff += MAX_SEQN;
                    } 
                    if (!(diff > WND_SIZE * PAYLOAD_SIZE)) {
                        int pos = (s + ((diff + PAYLOAD_SIZE - 1) / PAYLOAD_SIZE)) % WND_SIZE;
                        memcpy(&buf[pos], &recvpkt, PKT_SIZE);
                        valid[pos] = 1;
                        if (recvpkt.seqnum == cliSeqNum) {
                            // expected packet
                            valid[s] = 1;
                            while (valid[s]) {
                                fwrite(buf[s].payload, 1, buf[s].length, fp);
                                cliSeqNum = (cliSeqNum + buf[s].length) % MAX_SEQN;
                                valid[s] = 0;
                                s = (s + 1) % WND_SIZE;
                            }
                        }
